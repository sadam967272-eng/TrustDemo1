   document.addEventListener('DOMContentLoaded', function() {
    ...
    // API configuration لـGemini
    const API_KEY = "AIzaSyAYKXR9_azLMcZC_ec-e20Y8ozE0CX-M2Y"; // مفتاح API الخاص بك
    ...
    async function sendAiMessage() {
        ...
        try {
            ...
            aiTypingIndicator.classList.remove('active');
            addAiMessage("Gemini", "ذ", "#7e57c2", "عذراً، حدث خطأ في الاتصال بالذكاء الاصطناعي. يرجى المحاولة مرة أخرى.", false);
            console.error("خطأ في Gemini API:", error);
        }
    }
    
    // دالة إرسال الرسالة إلى Gemini API
    async function sendToGemini(message) {
        ...
        if (data.candidates && data.candidates.length > 0 && data.candidates[0].content && data.candidates[0].content.parts && data.candidates[0].content.parts.length > 0) {
            ...
            throw new Error('لم يتم العثور على رد صالح من الـAPI');
        }
    }
